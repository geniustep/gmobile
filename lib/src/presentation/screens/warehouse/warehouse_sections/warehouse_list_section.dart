...
import 'package:gsloution_mobile/src/data/models/warehouse_model/warehouse_model.dart';
...
class _WarehouseListSectionState extends State<WarehouseListSection> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      if (!hasWarehouses) {
        // إذا لم تكن هناك مستودعات من السيرفر، استخدم البيانات الثابتة
        return ListView.builder(
          itemCount: warehouseModel.length,
          itemBuilder: (context, index) {
            final warehouseData = warehouseModel[index];
            return _buildWarehouseCard(warehouseData, index, isStatic: true);
          },
        );
      }
            ...
            children: [
              Container(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    // TODO: Implement edit for StockWarehouseModel
                    if (kDebugMode) {
                      print('Edit warehouse: ${warehouse.id}');
                    }
                  },
                ),
              ),
  ...
  Widget _buildWarehouseCard(Map<String, dynamic> warehouseData, int index, {bool isStatic = false}) {
    return Container(
          margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          constraints:
